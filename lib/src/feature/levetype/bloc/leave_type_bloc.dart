  ...
  @override
    Stream<LeaveTypeState> mapEventToState(LeaveTypeEvent event) async*{
      ...
      if(event is FetchLeaveTypeStarted){
        ...
        try {
           List<LeaveTypeModel> _departmentList = await departmentRepository.getleaveType(
                rowPerpage: rowperpage, page: page);
      ...
      if(event is RefreshLeaveTypeStarted){
        ...
        try {
          ...
          if(leavetype.length !=0){
           ...
           List<LeaveTypeModel> leaveList =await departmentRepository.getleaveType(
                rowPerpage: rowperpage, page: page);
      ...
      if(event is AddLeaveTypeStarted){
        ...
        try {
          await departmentRepository.addLeaveType(name: event.name, note: event.note);
      ...
      if(event is UpdateLeaveTypeStarted){
        yield AddingLeaveType();
        try {
          await departmentRepository.editLeaveType(id: event.id,name: event.name, note: event.note);
          yield AddedLeaveType();
          yield FetchingLeaveType();
          print(leavetype.length);
          leavetype.clear();
